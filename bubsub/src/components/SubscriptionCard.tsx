...
export function SubscriptionCard({ sub }: { sub: Subscription }) {
  ...
  return (
    <Card onClick={() => navigate(`/subs/${sub.id}`)} className={`p-3.5 ${cancelled ? 'opacity-55' : ''}`}>
      <div className="flex items-center gap-3.5">
        <LogoTile name={sub.name} color={sub.color} serviceId={sub.serviceId} />
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h3 className="text-[15px] font-semibold text-ink-900 dark:text-ink-50 truncate">{sub.name}</h3>
          ...
          <p className="text-[12px] text-ink-500 dark:text-ink-400 truncate mt-0.5">
            {categoryLabel(sub.category)}
            {!cancelled && (
              <>
                {' · '}
                <span className={days <= 3 ? 'text-amber-600 dark:text-amber-300 font-medium' : ''}>
        ...
        <div className="text-right shrink-0">
          ...
          <p className="text-[10px] text-ink-400 dark:text-ink-500 font-medium uppercase tracking-wide">/mo</p>
        </div>
        <IconChevronRight className="w-4 h-4 text-ink-300 dark:text-ink-600 shrink-0 opacity-60" />
      </div>
    </Card>
  )
}
